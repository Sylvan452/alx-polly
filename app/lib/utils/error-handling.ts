...
type SecurityEventType =
  ...
  | 'UNAUTHORIZED_ADMIN_REVOKE_ATTEMPT'
  | 'ADMIN_SELF_REVOKE_ATTEMPT'
  | 'ADMIN_GRANT_FAILED'
  | 'ADMIN_REVOKE_FAILED'
  | 'ADMIN_GRANT_ERROR'
  | 'ADMIN_REVOKE_ERROR'
  | 'UNAUTHORIZED_ADMIN_LIST_ATTEMPT'
  | 'ADMIN_LIST_FAILED'
  | 'ADMIN_LIST_ACCESSED'
  | 'ADMIN_LIST_ERROR'
...
export function withErrorHandling<T extends any[], R>(
  ...
  return async (...args: T): Promise<R | { error: string }> => {
    try {
      ...
      const safeError = sanitizeError(error, context ? { ...context, timestamp: new Date().toISOString() } : undefined);
      return { error: safeError.message };
    }
  };
}

// Removed duplicate logSecurityEvent function - using the async version above
